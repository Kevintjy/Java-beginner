...
3. INITIAL DATA PART: There are a java file called CreateDataForTest, after you run it, the database is going to have 5 users, 10 accounts, 2 managers,
...
If the user want to make change to any account he has, he needs to enter "1" on his user's main page.
...
If the user want to send account creation request, he needs to enter "2" on his user's main page.
...
If the user want to make change to his user's password, he needs to enter "3" on his user's main page.
...
If the user want to check his total balance, he needs to enter "4" on his user's main page.
    Then the program will double check and see whether he wants to see the balance, and then the balance will be shown.
If the user want to change his primary account, then he needs to enter "5" on his user's main page.
...
If there is some request stored, the system will show the first request on the screen. You can choose to agree or reject that request.
 With your agreement, you will need to create account by entering account number. You can try again if you enter a invalid number.
...
9.4 Check flowchart.pdf to see how to send a request.


10：MAIN PART: after created data in the database, you can run main to implement our program, when the manager shut down the
program, all the data is going to write back to the data base. You can refer to the flow chart called " xxxxxxxxxxxx".

11. CONFIG_DATE: The last add interests date is 2019-03-01.
